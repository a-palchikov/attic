...
### Leaf types
## ap(g1004): aligned structs
subStruct = AlignedStruct4("substructs",
    leaf_type,
...
lfFieldList = Struct("lfFieldList",
    # ap(g1002): i have a .pdb with an empty field list, what does it make it? optional?..
    OptionalGreedyRange(subStruct)
    #GreedyRange(subStruct)
)

lfEnum = AlignedStruct4("lfEnum",
    ULInt16("count"),
    CV_property,
    ULInt32("utype"),
    ULInt32("fieldlist"),
...
lfBitfield = AlignedStruct4("lfBitfield",
    ULInt32("base_type"),
    ULInt8("length"),
    ULInt8("position"),
)

lfStructureST = AlignedStruct4("lfStructureST",
    ULInt16("count"),
    CV_property,
    ULInt32("fieldlist"),
    ULInt32("derived"),
...
lfStructure = AlignedStruct4("lfStructure",
    ULInt16("count"),
    CV_property,
    ULInt32("fieldlist"),
    ULInt32("derived"),
...
lfArray = AlignedStruct4("lfArray",
    ULInt32("element_type"),
    ULInt32("index_type"),
    val("size"),
)

lfArrayST = AlignedStruct4("lfArray",
    ULInt32("element_type"),
    ULInt32("index_type"),
    ULInt16("size"),
    PascalString("name"),
...
lfArgList = AlignedStruct4("lfArgList",
    ULInt32("count"),
    Array(lambda ctx: ctx.count, ULInt32("arg_type")),
)

lfProcedure = AlignedStruct4("lfProcedure",
    ULInt32("return_type"),
    CV_call,
    ULInt8("reserved"),
    ULInt16("parm_count"),
...
lfModifier = AlignedStruct4("lfModifier",
...
lfPointer = Struct("lfPointer",
    ...
    # ap(g1005): hardcoded until I figure out how to compute part of Struct dynamically
    #RestrictedPadAlign(8),  # 8 - sizeof(lfPointer), 2 - sizeof(leaf_type)
    If(lambda ctx: ctx._.length > 8,
            Padding(lambda ctx: ctx._.length - 8)
    ),
)

lfUnion = AlignedStruct4("lfUnion",
    ULInt16("count"),
    CV_property,
    ULInt32("fieldlist"),
    val("size"),
...
lfUnionST = AlignedStruct4("lfUnionST",
    ULInt16("count"),
    CV_property,
    ULInt32("fieldlist"),
    ULInt16("size"),
...
lfMFunc = AlignedStruct4("lfMFunc",
    ULInt32("return_type"),
    ULInt32("class_type"),
    ULInt32("this_type"),
    CV_call,
...
#Type = Struct("type",
    leaf_type,
    #Value("length", lambda ctx: ctx._.length - ctx.leaf_type.sizeof()), # ap(todo): arghhh! how do I sizeof(leaf_type)???
    Value("length", lambda ctx: ctx._.length - 2),
    Switch("type_info", lambda ctx: ctx.leaf_type,
        {
...
Type = Struct("types",
    ...
    #parse_callback = lambda con, subcon, parsed_value: subcon.name == "length" and parsed_value == 2 and pdb.set_trace(),
)
...
def resolve_typerefs(leaf, types, min):
    ...
    # ap: some leaf types are still missing type information (like 'LF_METHODLIST')
    # so filter these out to avoid KeyErrors

    if leaf.leaf_type in type_refs:
        for attr in type_refs[leaf.leaf_type]:
            # ap(g1006): arglists can be empty
            if attr == 'arg_type':
...
def merge_fwdrefs(leaf, types, map):
    if leaf.leaf_type in type_refs:
        ...
        #print 'merge_fwdref: no type information for leaf type %s' % leaf.leaf_type
        pass
    return leaf
